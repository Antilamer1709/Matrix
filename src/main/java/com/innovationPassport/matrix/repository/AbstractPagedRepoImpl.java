...
@Slf4j
public abstract class AbstractPagedRepoImpl<T, F> implements AbstractPagedRepo<T, F> {
    ...
    public PageImpl<T> getPagedData(SearchDTO<F> filter) {
        ...
        Query countQuery = makePagedQuery(filter.getFilter(), filter.toPageable(), true);
    ...
    protected Query makePagedQuery(F filter, Pageable pageable, boolean count) {
        ...
        if (!count) {
            if (pageable.getSort() != null)
                ...
                queryBuilder.append(getDefaultOrderBy());
        }
    ...
    protected String getDefaultOrderBy() {
        return " ORDER BY U." + this.idName + " DESC ";
    }
